...
dm9000_init(void)
{
    ...
    //  (oldval_bwscon & ~(3 << 16)) 清空原有的数据总线宽度位数。
    //重新配置bank4为16位数据总线，wait使能，使用UB/LB引脚(对应nBE[3:0])
    //*((volatile unsigned int *)S3C2410_BWSCON) = (oldval_bwscon & ~(0xf << 16)) | S3C2410_BWSCON_DW4_16 | S3C2410_BWSCON_WS4 | S3C2410_BWSCON_ST4;
    *((volatile unsigned int *)S3C2410_BWSCON) = (oldval_bwscon & ~(0xf << 16)) | S3C2410_BWSCON_DW4_16 | S3C2410_BWSCON_WS4;
    //Tacs=0x00(0个时钟), Tcos=0x11(4个时钟),Tacc=0x111(14个时钟),Tcoh=0x01(1个时钟),Tcah=0x11(4个时钟)
    //Tacp=0x11(4个时钟)，PMC=0x00(1个数据)
