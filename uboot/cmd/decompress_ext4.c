...
int write_raw_chunk(char* data, unsigned int sector, unsigned int sector_size) {
    ...
    sprintf(run_cmd, "nand erase 0x%x 0x%x", sector, sector_size);
	run_command(run_cmd, 0);

	sprintf(run_cmd, "nand write 0x%x 0x%x 0x%x", (int)((ulong)data), sector, sector_size);
	run_command(run_cmd, 0);
